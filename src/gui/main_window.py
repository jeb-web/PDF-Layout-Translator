...
class MainWindow:
    ...
    def _create_home_tab(self):
        ...
        ttk.Combobox(lang_frame, textvariable=self.source_lang_var, values=["auto", "fr", "en", "es", "de", "it"], width=8).pack(side='left', padx=(10, 0))
        ttk.Label(lang_frame, text="Langue cible:").pack(side='left', padx=(20, 0))
        self.target_lang_var = tk.StringVar(value="en")
        ttk.Combobox(lang_frame, textvariable=self.target_lang_var, values=["en", "fr", "es", "de", "it", "pt"], width=8).pack(side='left', padx=(10, 0))
        ...
        self.sessions_tree.heading('#0', text='Nom'); self.sessions_tree.heading('date', text='Date'); self.sessions_tree.heading('status', text='Statut'); self.sessions_tree.heading('progress', text='Progrès')
    ...
    def _create_analysis_tab(self):
        ...
        self.continue_to_translation_button = ttk.Button(buttons_frame, text="Continuer vers Traduction", command=self._continue_to_translation, state='disabled')
    ...
    def _create_translation_tab(self):
        ...
        instructions_text = "1. Cliquez sur \"Générer Export\"...\n2. Utilisez votre IA pour traduire...\n3. Collez la traduction...\n4. Cliquez sur \"Valider Traduction\"..."
        ...
        self.generate_export_button = ttk.Button(export_frame, text="Générer Export pour Traduction", command=self._generate_translation_export)
        self.generate_export_button.pack(side='left')
        self.open_export_folder_button = ttk.Button(export_frame, text="Ouvrir Dossier d'Export", command=self._open_export_folder, state='disabled')
        ...
        self.validate_translation_button = ttk.Button(validation_frame, text="Valider Traduction", command=self._validate_translation)
        ...
        ttk.Combobox(validation_frame, textvariable=self.validation_level_var, values=["strict", "moderate", "permissive"], width=10).pack(side='left', padx=(10, 0))
        self.continue_to_layout_button = ttk.Button(validation_frame, text="Continuer vers Mise en Page", command=self._continue_to_layout, state='disabled')
    ...
    def _create_layout_tab(self):
        ...
        ttk.Checkbutton(settings_frame, text="Autoriser la réduction de police", variable=self.allow_font_reduction_var).pack(anchor='w')
        self.allow_container_expansion_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(settings_frame, text="Autoriser l'expansion des conteneurs", variable=self.allow_container_expansion_var).pack(anchor='w')
        ...
        self.process_layout_button = ttk.Button(layout_buttons_frame, text="Traiter la Mise en Page", command=self._process_layout)
        self.process_layout_button.pack(side='left')
        self.preview_layout_button = ttk.Button(layout_buttons_frame, text="Aperçu", command=self._preview_layout, state='disabled')
        self.preview_layout_button.pack(side='left', padx=(10, 0))
        self.continue_to_export_button = ttk.Button(layout_buttons_frame, text="Continuer vers Export", command=self._continue_to_export, state='disabled')
    ...
    def _create_export_tab(self):
        ...
        ttk.Checkbutton(options_frame, text="Créer une sauvegarde du fichier original", variable=self.create_backup_var).pack(anchor='w')
        self.create_comparison_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Créer un PDF de comparaison", variable=self.create_comparison_var).pack(anchor='w')
        self.optimize_output_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Optimiser le fichier de sortie", variable=self.optimize_output_var).pack(anchor='w')
        ...
        self.export_pdf_button = ttk.Button(export_buttons_frame, text="Exporter PDF", command=self._export_pdf)
        self.export_pdf_button.pack(side='left')
        self.open_output_folder_button = ttk.Button(export_buttons_frame, text="Ouvrir Dossier de Sortie", command=self._open_output_folder, state='disabled')
        self.open_output_folder_button.pack(side='left', padx=(10, 0))
        self.new_project_button = ttk.Button(export_buttons_frame, text="Nouveau Projet", command=self._new_project, state='disabled')
    ...
    def _initialize_managers(self):
        try:
            ...
            self.logger.error(f"Erreur init: {e}", exc_info=True)
            messagebox.showerror("Erreur", f"Erreur lors de l'initialisation: {e}")

    def _load_recent_sessions(self):
        if not self.session_manager: return
        for item in self.sessions_tree.get_children(): self.sessions_tree.delete(item)
        sessions = self.session_manager.list_sessions()
        for session in sessions[-10:]:
            try:
                date_obj = datetime.fromisoformat(session.last_modified)
                date_str = date_obj.strftime("%d/%m/%Y %H:%M")
            except: date_str = "Inconnue"
            progress = int((session.translation_progress + session.review_progress) * 50)
            self.sessions_tree.insert('', 'end', text=session.name, values=(date_str, session.status.value, f"{progress}%"), tags=(session.id,))

    def _browse_pdf_file(self):
        filename = filedialog.askopenfilename(title="Sélectionner un PDF", filetypes=[("Fichiers PDF", "*.pdf")])
    ...
    def _start_new_project(self):
        ...
        try:
            session_id = self.session_manager.create_session(Path(pdf_path), source_lang=self.source_lang_var.get(), target_lang=self.target_lang_var.get())
            self.current_session_id = session_id
            self._update_session_info()
            self.notebook.select(1)
            self._update_global_progress(1, "Session créée, analyse...")
            self._analyze_pdf()
        except Exception as e:
    ...
    def _analyze_pdf(self):
        ...
        def analyze_thread():
            try:
                self._set_processing(True, "Analyse du PDF...")
                session_info = self.session_manager.get_session_info(self.current_session_id)
                analysis_data = self.pdf_analyzer.analyze_pdf(Path(session_info.original_pdf_path))
                ...
                self.root.after(0, lambda e=e: messagebox.showerror("Erreur", f"Erreur d'analyse: {e}"))
            finally:
                self._set_processing(False)
        threading.Thread(target=analyze_thread, daemon=True).start()

    def _display_analysis_results(self, data):
        info_text = f"Pages: {data['document_info']['page_count']}\nVersion PDF: {data['document_info']['pdf_version']}"
        self._update_text_widget(self.doc_info_text, info_text)
        stats = data['statistics']
        results_text = f"Éléments texte: {stats['total_text_elements']}\nCaractères: {stats['total_characters']}\nMots: {stats['total_words']}"
    ...
    def _generate_translation_export(self):
        ...
        def export_thread():
            try:
                ...
                with open(session_dir / "extraction_data.json", 'w', encoding='utf-8') as f: json.dump(extraction_data, f, indent=2)
                ...
                self.root.after(0, lambda e=e: messagebox.showerror("Erreur", f"Erreur d'export: {e}"))
            finally: self._set_processing(False)
        threading.Thread(target=export_thread, daemon=True).start()

    def _validate_translation(self):
        content = self.translation_input.get('1.0', tk.END).strip()
        if not content: return
        def validate_thread():
            try:
                self._set_processing(True, "Validation...")
                session_dir = self.session_manager.get_session_directory(self.current_session_id)
                with open(session_dir / "extraction_data.json", 'r', encoding='utf-8') as f: extraction_data = json.load(f)
                report = self.translation_parser.parse_translated_content(content, extraction_data)
                self.root.after(0, lambda: self._show_validation_results(report))
                if report.result.value in ['success', 'partial']:
                    validated = self.translation_parser.export_validated_translations(report)
                    with open(session_dir / "validated_translations.json", 'w', encoding='utf-8') as f: json.dump(validated, f, indent=2)
                ...
                self.root.after(0, lambda e=e: messagebox.showerror("Erreur", f"Erreur de validation: {e}"))
            finally: self._set_processing(False)
    ...
    def _process_layout(self):
        ...
        def layout_thread():
            try:
                self._set_processing(True, "Traitement mise en page...")
                session_dir = self.session_manager.get_session_directory(self.current_session_id)
                with open(session_dir / "analysis_data.json", 'r', encoding='utf-8') as f: analysis_data = json.load(f)
                with open(session_dir / "validated_translations.json", 'r', encoding='utf-8') as f: validated = json.load(f)
                result = self.layout_processor.process_layout(validated, analysis_data)
                with open(session_dir / "layout_result.json", 'w', encoding='utf-8') as f: json.dump(result, f, indent=2)
                self.root.after(0, lambda: self._display_layout_results(result))
                self.session_manager.update_session_status(SessionStatus.READY_FOR_EXPORT, self.current_session_id)
            except Exception as e:
                self.root.after(0, lambda e=e: messagebox.showerror("Erreur", f"Erreur de mise en page: {e}"))
            finally: self._set_processing(False)
    ...
    def _export_pdf(self):
        ...
        def export_thread():
            try:
                self._set_processing(True, "Exportation PDF...")
    ...
    def _open_selected_session(self):
        ...
        self.current_session_id = session_id
        self._update_session_info()
        messagebox.showinfo("Succès", "Session chargée ! Reprise du travail...")
        ...
        try:
            ...
            messagebox.showerror("Erreur de Reprise", f"Impossible de restaurer l'état: {e}")
    ...
    def _show_export_results(self, result, path): self._update_text_widget(self.export_results_text, f"Résultat: {'Succès' if result.success else 'Échec'}\nFichier: {path}"); self.new_project_button.config(state='normal')
